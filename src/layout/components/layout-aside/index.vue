<template>
	<el-aside class="aside" :class="{ close: isCollapse }">
		<div class="title" v-if="!isCollapse">
			<i class="el-icon-s-fold" @click="setSidebar(1)" />收起菜单
		</div>
		...
		>
			<template v-for="menu in menus0">
				...
				>
                    ...
                    <el-tooltip slot="title" :content="menu.meta.title" placement="right">
                        <span class="ml5">{{ menu.meta.title }}</span>
                    </el-tooltip>
				...
				<el-submenu v-else :key="menu.path" :index="menu.path">
					...
					>
                        ...
                        <el-tooltip slot="title" :content="item.meta.title" placement="right">
...
import { pathsBlank } from '@/router/menu'
export default {
	name: 'layout-aside',
	data() {
		return {}
	...
	methods: {
		...
		selectMenu(index) {
			// 新窗口打开
			if (pathsBlank.includes(index)) {
				let a = document.createElement('a')
				a.href = '#'+index // hash模式添加#
				a.target = '_blank'
				a.click()
